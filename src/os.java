/* 
 * This class represents an operating system that works with a program sos, which simulates a stream that sends jobs
 * and job requests.
...
public class os{
     ...
     *      If the job has a terminated status,
     ...
     * 
     * If the currently working job is NOT in core, the job is placed in core and onto 
     ...
     *        the job's terminated status is set to true.
     *     Otherwise, 
     ...
     *      remove the job from the main to drum queue
     *      if the job's IO count is greater than zero, set its terminated status to true
     *      if the job's IO count is zero, remove the job from memory and the job table
     *
     ...
     *              running job is not latched, the job is added to the main to drum queue
     ...
     *    Every job in drumToMainQueue is examined. The job is
     *    placed into memory if it can find an adequate space in the
